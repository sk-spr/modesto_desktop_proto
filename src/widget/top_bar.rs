...
impl Widget for TopBarButton {
    fn render(&mut self, width: usize, height: usize) -> Option<Vec<[u8; 4]>> {
        if !self.needs_redraw && !self.cache.is_empty() {
            ...
            self.cache = self.text.get_cache();
        }
    ...
    fn handle_mouse_event(
        ...
        relative_mouse_position: MousePosition,
        mouse_event: MouseEvent,
        registrar: &mut MouseCallbackRegistrar,
    ) {
        todo!()
    }
}
...
impl TopBarWidget {
    ...
    pub fn new(buttons: Vec<Box<dyn Widget>>) -> Self {
        TopBarWidget {
            buttons,
            cache: vec![],
            cache_width: 0,
...
impl Widget for TopBarWidget {
    ...
    fn handle_mouse_event(
        ...
        println!(
            "Handling top bar mouse event, abs x={};y={}; rel x={};y={}",
            mouse_position.x_position,
            mouse_position.y_position,
            relative_mouse_position.x_position,
            relative_mouse_position.y_position
        );
    }
}
